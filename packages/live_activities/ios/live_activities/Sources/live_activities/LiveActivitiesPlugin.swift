...
public class LiveActivitiesPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, FlutterSceneLifeCycleDelegate {
    ...
    func createActivity(data: [String: Any], removeWhenAppIsKilled: Bool, enableRemoteUpdates: Bool, staleIn: Int?, activityId: String? = nil, result: @escaping FlutterResult) {
        ...
        let initialRevision = readContentRevision(from: data)
    ...
    func updateActivity(activityId: String, data: [String: Any?], alertConfig: FlutterAlertConfig?, result: @escaping FlutterResult) {
        Task {
            ...
            let updatedRevision = readContentRevision(fromFlutterData: data)
    ...
    struct LiveActivitiesAppAttributes: ActivityAttributes, Identifiable {
        ...
        public struct ContentState: Codable, Hashable {
            ...
            /// Distinto en cada `update` para forzar repintado; opcional por compat. con estados antiguos sin clave.
    ...
    /// Valor distinto por actualización: ActivityKit suele ignorar `update(using:)` si el `ContentState` no cambia de `Hashable`.
    @available(iOS 16.1, *)
    private func readContentRevision(from data: [String: Any]) -> Int64 {
        if let n = data["iosPresentationTick"] as? NSNumber { return n.int64Value }
        if let n = data["iosPresentationTick"] as? Int { return Int64(n) }
        if let n = data["iosPresentationTick"] as? Int64 { return n }
        return Int64(Date().timeIntervalSince1970 * 1000)
    }
    
    @available(iOS 16.1, *)
    private func readContentRevision(fromFlutterData data: [String: Any?]) -> Int64 {
        var plain: [String: Any] = [:]
        plain.reserveCapacity(data.count)
        for (key, value) in data {
            guard let value = value, !(value is NSNull) else { continue }
            plain[key] = value
        }
        return readContentRevision(from: plain)
    }
